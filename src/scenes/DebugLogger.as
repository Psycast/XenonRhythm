package scenes 
{
	import assets.sGameBackground;
	import classes.engine.EngineCore;
	import classes.ui.UICore;
	import classes.ui.UIStyle;
	import flash.text.TextField;
	import flash.text.TextFieldAutoSize;

	public class DebugLogger extends UICore 
	{
		private var _tf:TextField;
		private var _colors:Array = ["#DDDDDD", "#FFFFFF", "#FFCE7F", "#FF5E5B", "#E7ADFF"];
		
		public function DebugLogger(core:EngineCore) 
		{
			super(core);
		}
		
		override public function init():void
		{
			var bg:sGameBackground = new sGameBackground();
			addChildAt(bg, 0);
			
			_tf = new TextField();
			_tf.x = 5;
			_tf.y = 5;
			_tf.width = Constant.GAME_WIDTH - 30;
			_tf.height = Constant.GAME_HEIGHT - 10;
			_tf.embedFonts = true;
			_tf.multiline = true;
			_tf.defaultTextFormat = UIStyle.getTextFormat();
			_tf.autoSize = TextFieldAutoSize.NONE;
			addChild(_tf);
		}
		
		override public function onStage():void
		{
			draw();
		}
		
		override public function draw():void
		{
			_tf.htmlText = buildHistory();
		}
		
		private function buildHistory():String 
		{
			//history.push([class_name(clazz), level, text, simple]);
			//trace(level + ":" + (!simple ? "[" + class_name(clazz) + "] " : "") + text);
			var a:Array = Logger.history;
			var b:Array;
			var s:String = "";
			for (var i:int = 0; i < a.length; i++)
			{
				b = a[i];
				s += "<font color=\"" + _colors[b[1]] + "\">" + (!b[3] ? "[" + b[0] + "] " : "") + b[2] + "</font><br>";
			}
			return s;
		}
	}

}