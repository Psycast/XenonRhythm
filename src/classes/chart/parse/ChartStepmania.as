package classes.chart.parse {
	...
	public class ChartStepmania extends NoteChart {
		...
		public function ChartStepmania(inData:String, framerate:int = 60):void {
			type = NoteChart.SM;
			frameOffset = -1;
			super(inData.split("\r"), framerate);
			
			var lineNum:int = 0;
			while (chartData[lineNum] != null) {
				var lineText:String = String(chartData[lineNum]);
				//- Set Gap
				if (lineText.substr(0, 4) == "#OFF") {	// #OFFSET
					this.gap = Number(lineText.substring(8, lineText.length - 1)) * -1;
				}
				
				//- Set BPM's
				if (lineText.substr(0, 4) == "#BPM") {	// #BPMS
					addBPMs(lineText.substring(6, lineText.length - 1));
				}
				
				//- Set Freeze's
				if (lineText.substr(0, 4) == "#STO") {	// #STOPS
					addFreezes(lineText.substring(7, lineText.length - 1));
				}
				
				//- Set Notes
				if (lineText.substr(0, 4) == "#NOT") {	// #NOTES
